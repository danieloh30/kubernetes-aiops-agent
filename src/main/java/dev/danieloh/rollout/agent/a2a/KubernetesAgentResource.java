package dev.danieloh.rollout.agent.a2a;
...
import dev.danieloh.rollout.agent.workflow.KubernetesWorkflow;
import dev.danieloh.rollout.agent.agents.RemediationAgent;
...
import dev.danieloh.rollout.agent.remediation.GitHubIssueTool;
import dev.danieloh.rollout.agent.remediation.GitHubRestClient;
import dev.danieloh.rollout.agent.remediation.SourceCodeTool;
import dev.danieloh.rollout.agent.utils.RateLimiter;
import dev.danieloh.rollout.agent.utils.RetryHelper;
import dev.danieloh.rollout.agent.utils.TokenManager;
import dev.danieloh.rollout.agent.utils.ToolCallLimiter;
...
public class KubernetesAgentResource {
                        ...
                        if ((remediationResult.prLink() == null || remediationResult.prLink().isEmpty())
                            ...
                            try {
                                ...
                                Log.info(MessageFormat.format(
                                    "Fallback GitHub issue created: {0}",
                                    issueUrl
                                ));
                            } catch (Exception issueEx) {
                                Log.error("Failed to create fallback GitHub issue", issueEx);
                            }
                        } else if (remediationResult.prLink() != null && !remediationResult.prLink().isEmpty()) {
                            Log.info(MessageFormat.format(
    ...
    private List<String> extractFilePathsFromStackTraces(String text) {
        ...
        // Match fully qualified class names (e.g., dev.danieloh.demo.MetricsResource)
        if (paths.isEmpty()) {
